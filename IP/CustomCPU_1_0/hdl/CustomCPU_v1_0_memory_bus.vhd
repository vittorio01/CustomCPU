...
entity CustomCPU_v1_0_memory_bus is
	...
	port (
        ...
        transmission_address: in std_logic_vector(31 downto 0);
...
begin
	...
	M_AXI_AWLEN	<= (others => '0'); --std_logic_vector( to_unsigned(C_M_AXI_BURST_LEN - 1, 8) ); for now transfers can be only in normal mode
	...
	--Burst LENgth is number of transaction beats, minus 1
	M_AXI_ARLEN	<= (others => '0'); --std_logic_vector( to_unsigned(C_M_AXI_BURST_LEN - 1, 8) );
	  ...
	  begin                                                                
	    if (rising_edge (M_AXI_ACLK)) then                                 
	      if (M_AXI_ARESETN = '0' or init_txn_pulse = '1') then                                   
	        ...
	        if (axi_awvalid = '0') then --and start_single_burst_write = '1'
	          axi_awvalid <= '1';                                          
	          axi_awaddr <= (others =>'0');
	          wa_current_state <= idle;
	          -- Once asserted, VALIDs cannot be deasserted, so axi_awvalid
	          -- must wait until transaction is accepted           
	        elsif (wa_current_state = idle and transmission_write_start='1') then
	          axi_awvalid <= '1';
	          axi_awaddr <= transmission_address;
	          wa_current_state <= valid;
	        elsif (wa_current_state = valid and M_AXI_AWREADY='1') then
	           axi_awvalid <= '0';
	           wa_current_state <= stop;
	        elsif (wa_current_state = stop and transmission_write_start='0') then
	           wa_current_state <= idle;    
	        end if;                                              
	      end if;                                                          
	    end if;                                                            
	...
	-- write data                
	  ...
	  begin                                                                             
	    if (rising_edge (M_AXI_ACLK)) then                                              
	      ...
	      elsif (w_current_state = idle and transmission_write_start = '1') then 
	        axi_wvalid <= '1';
	        axi_wdata <= transmission_data_in;
	        w_current_state <= valid;
	      elsif (w_current_state = valid and M_AXI_WREADY = '1') then 
	        axi_wvalid <= '0';
	        w_current_state <= stop;
	      elsif (w_current_state = stop and transmission_write_start = '1') then 
	        w_current_state <= idle;
	      end if;                                                                                                                                         
	    end if;                                                                         
	  end process;                                                                      
	  ...
	  begin                                                              
	    if (rising_edge (M_AXI_ACLK)) then                               
	     ...
	     -- If previously not valid , start next transaction             
	        ...
	        if (ra_current_state = idle and transmission_read_start='1') then
	          axi_arvalid <= '1';                                  
	          axi_araddr <= transmission_address;
	          ra_current_state <= valid;
	        elsif (ra_current_state = valid and M_AXI_ARREADY='1') then          
	          axi_arvalid <= '0';      
	          ra_current_state <= stop;
	        elsif (ra_current_state = stop and  transmission_read_start = '0') then 
	          ra_current_state <= idle;                                
	        end if;                                                      
	      end if;                                                        
	    end if;                                                          
	...
	--Check received read data against data generator                       
	  ...
	  begin                                                                 
	    if (rising_edge (M_AXI_ACLK)) then                                  
	      if (M_AXI_ARESETN = '0') then             
	        transmission_data_out <= (others => '0');
